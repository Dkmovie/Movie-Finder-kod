...
async def cb_delete_handler(c,m):
    ...
    try:
        ...
        txt = await m.message.edit(f"Deleted Successfully")
    except Exception as e:
        print(e)
        txt = await m.message.edit(f"Some error occurred while deleting")
...
async def next_btn_cb_handler(client: Client, query: CallbackQuery):
    ...
    if results is not None:
        ...
        list2.append(
            [InlineKeyboardButton(text="BACK",callback_data=f"back_{int(offset/10)-1}_{keyword}")],
        )      

        if int((offset/10)+1) < math.ceil(len(results) / 10):
            
            list2.append(
                [InlineKeyboardButton(text="NEXT ⏩",callback_data=f"next_{int(offset/10)+1}_{keyword}")],
            )  
        ...
        txt = await query.edit_message_caption(caption=f"Results for {keyword}", reply_markup=reply_markup)
